...
'===== Outlook: Module "modExcelBridge" =====
Option Explicit

' ===== 32/64bit 両対応の前面化 API =====
#If VBA7 Then
...
' ====== 環境に合わせて設定 ======
Private Const SCHEDULE_BOOK_NAME As String = "Book1.xlsm"   ' Excel のブック名
Private Const SCHEDULE_BOOK_PATH As String = "C:\Users\kosuk\Desktop\Book1.xlsm"  ' 未オープン時のフルパス（空でもOK）

' ====== Excel 側で公開しているマクロ名 ======
Private Const EXCEL_MACRO_SET_PENDING As String = "modOutlookBridge_OL.SetPendingOutlookFromOutlook"
Private Const EXCEL_MACRO_SHOW_FORM As String = "modAutomation.Automation_ShowForm_Blank"
Private Const EXCEL_MACRO_SHOW_FORM_COMMENT As String = "modAutomation.Automation_ShowForm_Blank_ThenComment"

'==========================================
' 通常版：予定フォームだけ表示
'==========================================
Public Sub AddSchedule_FromSelectedMail()
    Call AddScheduleCore(False)
End Sub

'==========================================
' コメント付き版：予定→直後にコメントフォームも表示
'==========================================
Public Sub AddSchedule_FromSelectedMail_WithComment()
    Call AddScheduleCore(True)
End Sub

'------------------------------------------
' 共通本体
'------------------------------------------
Private Sub AddScheduleCore(ByVal withComment As Boolean)
    On Error GoTo EH
    ...
    If sel Is Nothing Or sel.Count = 0 Then
        MsgBox "先に対象のメールを選択してください。", vbExclamation
        Exit Sub
    End If
    Dim itm As Object: Set itm = sel(1)

    Dim entryId As String, storeId As String
    entryId = GetEntryID_Generic(itm)
    storeId = GetStoreID_Generic(itm)
    If Len(entryId) = 0 Or Len(storeId) = 0 Then
        MsgBox "この種類のアイテムはサポートされていません。", vbExclamation
        Exit Sub
    End If

    ' 2) Excel 起動/取得 & ブック取得
    Dim xlApp As Object, xlWb As Object
    Set xlApp = GetOrCreateExcel()
    Set xlWb = GetOrOpenWorkbook(xlApp, SCHEDULE_BOOK_NAME, SCHEDULE_BOOK_PATH)
    If xlWb Is Nothing Then Exit Sub

    ' 3) Excel を前面へ
    BringExcelToFront xlApp

    ' 4) Outlook の ID を渡し → フォーム起動
    xlApp.Run "'" & xlWb.Name & "'!" & EXCEL_MACRO_SET_PENDING, entryId, storeId
    If withComment Then
        xlApp.Run "'" & xlWb.Name & "'!" & EXCEL_MACRO_SHOW_FORM_COMMENT
    Else
        xlApp.Run "'" & xlWb.Name & "'!" & EXCEL_MACRO_SHOW_FORM
    End If
    Exit Sub

EH:
    MsgBox "処理中にエラーが発生しました: " & Err.Number & vbCrLf & Err.Description, vbCritical
End Sub

'------------------------------------------
' Excel Application を取得/起動
'------------------------------------------
Private Function GetOrCreateExcel() As Object
    Dim app As Object
    ...
    End If
    app.Visible = True
    Set GetOrCreateExcel = app
End Function

'------------------------------------------
' ブックを取得 or 開く（パス未設定/不正ならダイアログ）
'------------------------------------------
Private Function GetOrOpenWorkbook(ByVal xlApp As Object, ByVal wbName As String, ByVal wbPath As String) As Object
    Dim wb As Object, i As Long

    ' 既に開いているか？
    For i = 1 To xlApp.Workbooks.Count
        If StrComp(xlApp.Workbooks(i).Name, wbName, vbTextCompare) = 0 Then
    ...
    If wb Is Nothing Then
        ...
        If Len(pick) = 0 Or Dir$(CStr(pick)) = "" Then
            xlApp.Visible = True
            pick = xlApp.GetOpenFilename( _
                    "Excel マクロ有効ブック (*.xlsm;*.xlsb),*.xlsm;*.xlsb", , _
                    "予定表ブック（.xlsm または .xlsb）を選択してください")
            If VarType(pick) = vbBoolean Then
                MsgBox "ブックが選択されませんでした。", vbExclamation
                Set GetOrOpenWorkbook = Nothing
                Exit Function
            End If
        End If

        Set wb = xlApp.Workbooks.Open(fileName:=CStr(pick), ReadOnly:=False)
    End If
...
' Excel を前面へ
'------------------------------------------
Private Sub BringExcelToFront(ByVal xlApp As Object)
    On Error Resume Next
    If IsIconic(xlApp.hWnd) <> 0 Then ShowWindow xlApp.hWnd, SW_RESTORE
    SetForegroundWindow xlApp.hWnd
    On Error GoTo 0
End Sub

'------------------------------------------
' Outlook アイテムの EntryID / StoreID
'------------------------------------------
Private Function GetEntryID_Generic(ByVal itm As Object) As String
    On Error Resume Next
    GetEntryID_Generic = itm.entryId
End Function

Private Function GetStoreID_Generic(ByVal itm As Object) As String
    On Error Resume Next
    Dim fldr As Object
    Set fldr = itm.Parent ' MAPIFolder
    If Not fldr Is Nothing Then GetStoreID_Generic = fldr.storeId
End Function
...
Private Sub UserForm_Initialize()
    ...
    Set lbl = Me.Controls.Add("Forms.Label.1", "lblInfo", True)
    With lbl
        .caption = "Enter = 挿入 / Shift+Enter または Alt+Enter = 改行"
        .Left = 12
        .Top = 12
    ...
    Set txt = Me.Controls.Add("Forms.TextBox.1", "txtComment", True)
    With txt
        .Left = 12
        .Top = lbl.Top + lbl.Height + 6
        .Width = Me.InsideWidth - 24
        .Height = Me.InsideHeight - 80
        .Multiline = True
        .WordWrap = True
        .EnterKeyBehavior = False   ' Enter は自前で処理
...
Private Sub btnOK_Click()
    ...
    If Len(s) = 0 Then
        ...
        Exit Sub
    End If
    modQuickComment.InsertCommentToSelection s
    Me.Hide
End Sub
...
'=== modAutomation.bas への追加 ===
Public Sub Automation_ShowForm_Blank_ThenComment()
    modCalendarRange.ShowCalendarForm_WithComment
End Sub
...
Private Const TARGET_SHEET_NAME As String = "" ' ""=アクティブシート
Private Const TARGET_COL As Long = 2           ' B列
Private Const START_ROW As Long = 6            ' B6から

'---------------------------------------------
' 追加：予定フォーム→OK後に「コメントフォーム」も出す版
'---------------------------------------------
Public Sub ShowCalendarForm_WithComment()
    On Error Resume Next
    Unload frmCalendarRange
    On Error GoTo 0
    ...
    If frmCalendarRange.ClickedOK Then
        Dim memo As String: memo = Trim$(frmCalendarRange.MemoText)
        Dim d1 As Date: d1 = frmCalendarRange.SelectedStart
        Dim d2 As Date: d2 = frmCalendarRange.SelectedEnd
        Dim cat As Long: cat = frmCalendarRange.SelectedCategory

        ' ★コメントも出すフラグ True
        WriteMemoAndRange memo, d1, d2, cat, True
    End If
...
'---------------------------------------------
' 次の空行へ追加（コメントフォームを出すなら withComment:=True）
'---------------------------------------------
Public Sub WriteMemoAndRange(ByVal memo As String, ByVal d1 As Date, ByVal d2 As Date, ByVal cat As Long, _
                             Optional ByVal withComment As Boolean = False)

    Dim ws As Worksheet: Set ws = ResolveTargetSheet(TARGET_SHEET_NAME)
    Dim r As Long, T As Date
    ...
    ' 2.5) Outlookリンクを直付与（保留があれば）
    modOutlookBridge_OL.TryAttachPendingOutlookLink_ToRow ws, r

    ' 2.8) ★コメントフォームを出す（Bセルを選択してから）
    If withComment Then
        Application.Goto ws.Cells(r, "B"), True
        On Error Resume Next
        modQuickComment.ShowQuickCommentForm ' ← 既存のコメントフォーム
        On Error GoTo 0
    End If

    ' 3) D/E列：開始/終了（日付補完＋大小入替）
    If d1 <> 0 Or d2 <> 0 Then
        If d1 = 0 Then d1 = d2
        If d2 = 0 Then d2 = d1
        If d2 < d1 Then T = d1: d1 = d2: d2 = T
        With ws
            .Cells(r, "D").Value = d1: .Cells(r, "D").NumberFormatLocal = DATE_FMT_015
            .Cells(r, "E").Value = d2: .Cells(r, "E").NumberFormatLocal = DATE_FMT_015
        End With
    End If
...
CleanUp:
    ...
    SetupPeriodConditionalFormatting_020
    ApplyCategoryDividers_020 ws
    ThisWorkbook.GreyOutEveryCalendarSheet
End Sub

'---------------------------------------------
' 指定行に上書き（コメントフォーム出すなら withComment:=True）
'---------------------------------------------
Public Sub WriteMemoAndRange_ToRow(ByVal ws As Worksheet, ByVal r As Long, _
                                   ByVal memo As String, ByVal d1 As Date, ByVal d2 As Date, ByVal cat As Long, _
                                   Optional ByVal withComment As Boolean = False)
    Dim T As Date
    ...
    ' 1.8) ★コメントフォーム（Bセルを選んでから）
    If withComment Then
        Application.Goto ws.Cells(r, "B"), True
        On Error Resume Next
        modQuickComment.ShowQuickCommentForm
        On Error GoTo 0
    End If

    ' 2) D/E（日付補完＋大小入替）
    If Not (d1 = 0 And d2 = 0) Then
        If d1 = 0 Then d1 = d2
        If d2 = 0 Then d2 = d1
        If d2 < d1 Then T = d1: d1 = d2: d2 = T
...
Private Function FirstEmptyRow(ByVal ws As Worksheet, ByVal col As Long, ByVal startRow As Long) As Long
    ...
    Do While (LenB(ws.Cells(r, "B").Value2) <> 0) _
          Or (LenB(ws.Cells(r, "D").Value2) <> 0) _
          Or (LenB(ws.Cells(r, "E").Value2) <> 0)
        r = r + 1
    Loop
    FirstEmptyRow = r
End Function
...
Private Function ResolveTargetSheet(ByVal sheetName As String) As Worksheet
    ...
    Else
        ...
        On Error GoTo 0
        If ResolveTargetSheet Is Nothing Then Set ResolveTargetSheet = ActiveSheet
    End If
End Function
...
' ===== Excel: Module "modOutlookBridge_OL" =====
Option Explicit

' 一時保管（Outlook → Excel受け渡し用）
Private Const PENDING_SHEET As String = "__OL_PENDING__"
Private Const COL_ENTRYID As Long = 1
Private Const COL_STOREID As Long = 2

' 永続マップ（トークン→EntryID/StoreID）
Private Const MAP_SHEET As String = "__OL_LINKMAP__"
Private Const MAP_COL_TOKEN As Long = 1
Private Const MAP_COL_ENTRYID As Long = 2
Private Const MAP_COL_STOREID As Long = 3

'--------------------------------------------
' Outlook から呼ばれる：選択メールの ID を一時保存
'--------------------------------------------
Public Sub SetPendingOutlookFromOutlook(ByVal entryId As String, ByVal storeId As String)
    Dim ws As Worksheet
    Set ws = EnsurePendingSheet()
    ws.Cells(1, COL_ENTRYID).Value2 = entryId
    ws.Cells(1, COL_STOREID).Value2 = storeId
End Sub

Public Function HasPendingOutlookLink_OL() As Boolean
    Dim ws As Worksheet
    On Error Resume Next
    Set ws = ThisWorkbook.Worksheets(PENDING_SHEET)
    On Error GoTo 0
    HasPendingOutlookLink_OL = Not ws Is Nothing _
        And Len(CStr(ws.Cells(1, COL_ENTRYID).Value2)) > 0 _
        And Len(CStr(ws.Cells(1, COL_STOREID).Value2)) > 0
End Function

Public Function GetPendingEntryID_OL() As String
    On Error Resume Next
    GetPendingEntryID_OL = ThisWorkbook.Worksheets(PENDING_SHEET).Cells(1, COL_ENTRYID).Value2
End Function

Public Function GetPendingStoreID_OL() As String
    On Error Resume Next
    GetPendingStoreID_OL = ThisWorkbook.Worksheets(PENDING_SHEET).Cells(1, COL_STOREID).Value2
End Function

Public Sub ClearPendingOutlookLink_OL()
    On Error Resume Next
    With ThisWorkbook.Worksheets(PENDING_SHEET)
        .Cells(1, COL_ENTRYID).Value2 = vbNullString
        .Cells(1, COL_STOREID).Value2 = vbNullString
    End With
End Sub

'--------------------------------------------
' B列に Outlook リンクを作成（トークン方式／自セル参照）
'--------------------------------------------
Public Sub AddOutlookHyperlinkToCell_OL(ByVal tgt As Range, ByVal entryId As String, ByVal storeId As String, Optional ByVal caption As String = "")
    If caption = "" Then caption = CStr(tgt.Value)
    Dim token As String: token = NewToken_OL()
    ' マップに保存
    Map_Set_OL token, entryId, storeId

    ' 既存リンクを消してから追加
    On Error Resume Next
    tgt.Hyperlinks.Delete
    On Error GoTo 0

    ' 有効なセル参照（自分自身）を SubAddress に、トークンは ScreenTip に格納
    tgt.Worksheet.Hyperlinks.Add Anchor:=tgt, Address:="", _
        SubAddress:="'" & tgt.Worksheet.Name & "'!" & tgt.Address(False, False), _
        ScreenTip:="OLID:" & token, _
        TextToDisplay:=caption
End Sub

' 行番号が分かっているときに呼ぶ（B列直後に使う）
Public Sub TryAttachPendingOutlookLink_ToRow(ByVal ws As Worksheet, ByVal rowIndex As Long)
    If HasPendingOutlookLink_OL() Then
        Dim c As Range
        Set c = ws.Cells(rowIndex, "B")
        If Len(CStr(c.Value)) > 0 Then
            AddOutlookHyperlinkToCell_OL c, GetPendingEntryID_OL(), GetPendingStoreID_OL()
            ClearPendingOutlookLink_OL
        End If
    End If
End Sub

'--------------------------------------------
' クリックされたトークンから ID を取り出す（公開）
'--------------------------------------------
Public Function Map_GetByToken_OL(ByVal token As String, ByRef entryId As String, ByRef storeId As String) As Boolean
    Dim ws As Worksheet, f As Range
    Set ws = EnsureMapSheet()
    Set f = ws.Columns(MAP_COL_TOKEN).Find(What:=token, LookAt:=xlWhole, LookIn:=xlValues)
    If Not f Is Nothing Then
        entryId = CStr(ws.Cells(f.Row, MAP_COL_ENTRYID).Value2)
        storeId = CStr(ws.Cells(f.Row, MAP_COL_STOREID).Value2)
        Map_GetByToken_OL = (Len(entryId) > 0)
    Else
        Map_GetByToken_OL = False
    End If
End Function
...
' Outlook を ID で開く
'--------------------------------------------
Public Sub OpenOutlookMailByID(ByVal entryId As String, ByVal storeId As String)
    On Error Resume Next
    ...
    If Not itm Is Nothing Then
        itm.Display
    Else
        MsgBox "該当するメールを開けませんでした。移動/削除の可能性があります。", vbExclamation
    End If
End Sub

'==== 内部：保管シート／マップシート／トークン ====
Private Function EnsurePendingSheet() As Worksheet
    ...
    If ws Is Nothing Then
        ...
        ws.Name = PENDING_SHEET
        ws.Visible = xlSheetVeryHidden
    End If
    Set EnsurePendingSheet = ws
End Function

Private Function EnsureMapSheet() As Worksheet
    Dim ws As Worksheet
    On Error Resume Next
    Set ws = ThisWorkbook.Worksheets(MAP_SHEET)
    On Error GoTo 0
    If ws Is Nothing Then
        Set ws = ThisWorkbook.Worksheets.Add(After:=ThisWorkbook.Worksheets(ThisWorkbook.Worksheets.Count))
        ws.Name = MAP_SHEET
        ws.Cells(1, MAP_COL_TOKEN).Value = "Token"
        ws.Cells(1, MAP_COL_ENTRYID).Value = "EntryID"
        ws.Cells(1, MAP_COL_STOREID).Value = "StoreID"
        ws.Visible = xlSheetVeryHidden
    End If
    Set EnsureMapSheet = ws
End Function

Private Sub Map_Set_OL(ByVal token As String, ByVal entryId As String, ByVal storeId As String)
    Dim ws As Worksheet, f As Range, r As Long
    Set ws = EnsureMapSheet()
    Set f = ws.Columns(MAP_COL_TOKEN).Find(What:=token, LookAt:=xlWhole, LookIn:=xlValues)
    If f Is Nothing Then
        r = ws.Cells(ws.Rows.Count, MAP_COL_TOKEN).End(xlUp).Row + 1
    Else
        r = f.Row
    End If
    ws.Cells(r, MAP_COL_TOKEN).Value2 = token
    ws.Cells(r, MAP_COL_ENTRYID).Value2 = entryId
    ws.Cells(r, MAP_COL_STOREID).Value2 = storeId
End Sub

Private Function NewToken_OL() As String
    ' 重複しにくい短いトークンを生成
    Dim s As String
    Randomize
    s = Right(CStr(Abs(CLng(Rnd() * 1000000000#))), 9)
    NewToken_OL = "OL" & Format(Now, "yymmddhhnnss") & s
End Function
...
' エントリーポイント（日本語名）
Public Sub コメント入力フォーム()
    ShowQuickCommentForm
End Sub

' エントリーポイント（英語名）
Public Sub ShowQuickCommentForm()
    If TypeName(Selection) <> "Range" Then
    ...
    On Error Resume Next
    Unload frmQuickComment   ' 前回の残りを念のため破棄
    On Error GoTo 0

    frmQuickComment.Show vbModal

    On Error Resume Next
    Unload frmQuickComment
    On Error GoTo 0
End Sub

' フォームから呼ばれる：選択範囲の各セルにコメントを入れる
Public Sub InsertCommentToSelection(ByVal textIn As String)
    Dim rng As Range, c As Range
    If TypeName(Selection) <> "Range" Then Exit Sub
    Set rng = Selection

    ' コメント用の改行を安定化（メモは LF を好む）
    Dim s As String
...
' 1セルにコメントを追加（スレッドコメント→ダメならメモ）
Private Sub AddOrReplaceComment(ByVal tgt As Range, ByVal s As String)
    On Error Resume Next

    ' 既存のスレッドコメントを消す（ある場合）
    Err.Clear
    Dim tc As Object
    Set tc = CallByName(tgt, "CommentThreaded", VbGet)
    If Err.Number = 0 Then
        If Not tc Is Nothing Then CallByName tc, "Delete", VbMethod
    Else
        Err.Clear
    End If
    ...
    ' スレッドコメントで追加を試す
    Err.Clear
    CallByName tgt, "AddCommentThreaded", VbMethod, s
    If Err.Number = 0 Then Exit Sub

    ' 失敗したらメモ（旧コメント）にフォールバック
    Err.Clear
...
Private Sub Workbook_SheetFollowHyperlink(ByVal Sh As Object, ByVal Target As Hyperlink)
    Dim tip As String, token As String
    tip = Target.ScreenTip                 ' 例: "OLID:OL240316121530123456"
    If Left$(tip, 5) = "OLID:" Then
        token = Mid$(tip, 6)
        Dim eid As String, sid As String
        If modOutlookBridge_OL.Map_GetByToken_OL(token, eid, sid) Then
            modOutlookBridge_OL.OpenOutlookMailByID eid, sid
        End If
        ' SubAddress は自セル参照なのでエラーは出ません
    End If
End Sub
